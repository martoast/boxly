...
console.log('Spike 1: media.ready timing payload is ignored by the reader')
{
  ...
  const five = parseEventV1(frame({ capture_ms: 6, api_created_ms: 1264, children_ms: 1274, first_frame_ms: 4490, connected_ms: 3510 }), 'sess-9')
  check('media.ready with the five timing keys (capture_ms first, start-up overlap) parses and maps the same', five !== null && eventActivity(five.type) === 'media_ready' && five.payload.capture_ms === 6)
