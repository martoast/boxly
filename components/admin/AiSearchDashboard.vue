...
<script setup>
// The AI concierge analytics dashboard. Rendered by two pages that differ only
// in who is allowed in: /app/admin/ai-search (admins) and
// /app/shopping/ai-search (admins + the shopping team). `apiBase` picks the
// matching API namespace — both mount the same read-only controller.
...
async function downloadCsv() {
  ...
  try {
    ...
    const blob = await $customFetch(`${props.apiBase}/ai-search/export`, { params: { days: 0 }, responseType: 'blob' })
...
async function loadFeed(page = 1) {
  ...
  try {
    const res = await $customFetch(`${props.apiBase}/ai-search/events`, {
...
async function openThread(r) {
  ...
  thread.value = null
  try {
    thread.value = (await $customFetch(`${props.apiBase}/ai-search/thread/${r.conversation_id}`)).data
  } catch (e) {
    console.error(e)
...
const GALLERY_TOOLS = new Set(['search_products', 'curate_products', 'show_collection', 'browse_store', 'browse_stores', 'show_products', 'show_saved_products', 'find_live_product', 'show_orders'])
...
async function load() {
  loading.value = true
  const stats$ = $customFetch(`${props.apiBase}/ai-search/stats`, { params: { days: days.value, light: 1 } })
