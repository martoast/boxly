  ...
  <form @submit.prevent="onSubmit" class="space-y-6">
      ...
      <div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1 space-y-2">
            ...
            <div v-if="genderId" class="flex items-center gap-2">
    ...
    <div class="flex justify-end gap-3">
      ...
      <button type="submit" :disabled="saving" class="px-6 py-2.5 bg-primary-500 hover:bg-primary-600 disabled:bg-gray-300 text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-colors">
        {{ saving ? 'Guardando...' : (genderId ? 'Guardar cambios' : 'Crear género') }}
      </button>
    </div>
  </form>
</template>
...
const props = defineProps({
  existingGender: { type: Object, default: null },
})
const emit = defineEmits(['submit', 'upload-image'])

const route = useRoute()
const cancelTo = computed(() =>
  route.path.includes('/shopping/') ? '/app/shopping/genders' : '/app/admin/genders'
)

const genderId = computed(() => props.existingGender?.id)
