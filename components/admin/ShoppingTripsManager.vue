<template>
  <!-- CRUD for in-person shopping trips. The customer-facing date picker on
       /in-person reads the open trips this creates.

       Mounted by BOTH /app/admin/shopping-trips and
       /app/shopping/shopping-trips — Velonie runs the trips, so she owns the
       calendar. `apiBase` decides which namespace the writes go to; the
       controller behind them is the same one, so the two views can never
       drift apart. Single-file with an inline modal — see the sibling
       AiSearchDashboard for the same shape. -->
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="trip in trips" :key="trip.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-right whitespace-nowrap">
                <NuxtLink :to="`${basePath}/${trip.id}`" class="text-indigo-600 font-medium hover:text-indigo-700 text-sm mr-3">Ver citas</NuxtLink>
...
// '/admin' for Alex and Erick, '/shopping' for the shopping team. Both
// namespaces mount AdminShoppingTripsController, so the capability is
// identical — only the gate in front of it differs.
const props = defineProps({
  apiBase: { type: String, default: '/admin' },
})

// The page namespace mirrors the API one, so "Ver citas" stays inside the
// section the user is already in (a shopping employee following an /app/admin
// link would be bounced to /login by the admin middleware).
const basePath = computed(() => `/app${props.apiBase}/shopping-trips`)
...
async function fetchTrips() {
  loading.value = true
  try {
    const res = await $customFetch(`${props.apiBase}/shopping-trips`, {
...
async function save() {
  ...
  try {
    ...
    if (editingId.value) {
      await $customFetch(`${props.apiBase}/shopping-trips/${editingId.value}`, { method: 'PUT', body: payload })
      $toast.success('Visita actualizada')
    } else {
      await $customFetch(`${props.apiBase}/shopping-trips`, { method: 'POST', body: payload })
...
async function confirmDelete(trip) {
  ...
  try {
    await $customFetch(`${props.apiBase}/shopping-trips/${trip.id}`, { method: 'DELETE' })
