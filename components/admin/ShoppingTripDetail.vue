<template>
  <!-- One trip's confirmed bookings: who is coming, which stores, and a
       one-tap WhatsApp to coordinate. Mounted by both /app/admin and
       /app/shopping — see ShoppingTripsManager for the apiBase contract. -->
  <section class="min-h-screen bg-gray-50">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">

      <NuxtLink :to="basePath" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 mb-5">
...
onMounted(async () => {
  try {
    const res = await $customFetch(`${props.apiBase}/shopping-trips/${route.params.id}`)
