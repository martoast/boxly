<template>
  ...
  <div v-if="collapsed" class="flex flex-col items-center h-full py-3 gap-1">
    ...
    <button v-if="false" @click="$emit('memory')" title="Tu perfil de compras" aria-label="Tu perfil de compras" class="w-10 h-10 grid place-items-center rounded-lg text-primary-500 hover:bg-gray-100 active:scale-90 transition-all">
  ...
  <div v-else class="flex flex-col h-full">
    <div class="p-3 space-y-2">
      ...
      <button v-if="false" @click="$emit('memory')" class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-gray-200 hover:bg-gray-50 active:scale-[.98] text-gray-700 text-sm font-semibold transition-all">
