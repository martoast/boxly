  ...
  <div v-else class="flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
      <div
        ...
        :key="c.id"
        :class="['group flex items-center gap-1 pr-1.5 rounded-lg text-sm transition-colors', activeId === c.id ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100']"
      >
        <button
          type="button"
          class="min-w-0 flex-1 text-left pl-3 py-2 rounded-lg"
          :aria-label="`Abrir conversación: ${conversationTitle(c)}`"
          :aria-current="activeId === c.id ? 'page' : undefined"
          @click="$emit('open', c.id)"
        >
          <span class="block truncate">{{ c.title || 'Nuevo chat' }}</span>
        </button>
        <button
          type="button"
          ...
          :aria-label="`Eliminar conversación: ${conversationTitle(c)}`"
        >
...
function conversationTitle(conversation) {
  ...
  return (title || 'Nuevo chat').slice(0, 80)
}
</script>
