<template>
 ...
 <span v-for="s in pickedStores" :key="s.id" class="px-3 py-1 bg-primary-50 text-primary-700 rounded-full text-xs font-medium">{{ s.name }}</span>
 </div>
 </div>
 ...
 <!-- Pricing -->
 <div class="relative rounded-2xl bg-gradient-to-br from-primary-600 to-primary-500 text-white p-5 overflow-hidden">
 <h3 class="text-sm font-semibold opacity-90 uppercase tracking-wider">{{ t.pricingSection }}</h3>
 <div class="mt-3 space-y-1.5 text-sm">
 <div class="flex justify-between"><span class="opacity-85">{{ selectedStoreIds.length }} × $10 {{ t.perStore }}</span><span class="font-bold text-white">${{ serviceFee.toFixed(2) }} USD</span></div>
 ...
 @click="submit"
 :disabled="submitting"
 class="w-full inline-flex items-center justify-center gap-2 py-4 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-colors disabled:opacity-60"
 >
