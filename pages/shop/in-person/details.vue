<template>
  <!-- Step 3: minimum budget, categories, wishlist + notes. Budget is the
       only required input — wishlist and categories are optional context
       so admin can plan the trip, but customers can also schedule a
       free-form visit and trust Boxly to find good deals. -->
  <section class="min-h-screen bg-gray-50 pb-32">
    ...
    <div class="max-w-3xl mx-auto px-4 py-6 space-y-5">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        <div v-if="categoriesLoading" class="text-sm text-gray-400">{{ t.loading }}</div>
    ...
    <div class="fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 px-4 py-4 shadow-lg">
      <div class="max-w-3xl mx-auto">
        ...
        <button v-else disabled class="w-full py-3.5 bg-gray-300 text-white font-bold rounded-xl cursor-not-allowed">{{ t.budgetRequired }}</button>
      </div>
    </div>
  </section>
</template>
...
const {
  ...
  setBudget, setNotes, toggleCategory, addWishlistItem, removeWishlistItem,
} = useInPersonRequest()

const t = createTranslations({
  back:            { es: 'Volver', en: 'Back' },
  title:           { es: '¿Qué estás buscando?', en: "What are you looking for?" },
  subtitle:        { es: 'Cuéntanos más para que sepamos qué buscar.', en: "Tell us more so we know what to look for." },
  ...
  categoriesLabel: { es: 'Categorías de interés', en: 'Categories of interest' },
  categoriesHint:  { es: 'Marca las que te interesen para que nos enfoquemos en ellas.', en: 'Tap the ones you\'re interested in so we focus there.' },
...
onMounted(async () => {
  // Guard: prior steps must be done.
