<template>
  ...
  <section class="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
    <div class="max-w-md w-full bg-white rounded-3xl shadow-lg p-8 text-center">
      <div class="w-16 h-16 mx-auto mb-5 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white">
        <svg class="w-8 h-8" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
      </div>
      <h1 class="text-2xl font-extrabold text-gray-900">{{ t.title }}</h1>
      <p class="text-sm text-gray-600 mt-2">{{ t.subtitle }}</p>

      <div v-if="prRef" class="mt-5 inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 rounded-full">
        <span class="text-xs text-indigo-700 uppercase tracking-wider font-semibold">{{ t.refLabel }}</span>
        <span class="text-sm font-mono font-bold text-indigo-900">{{ prRef }}</span>
      </div>

      <p class="text-xs text-gray-500 mt-6 leading-relaxed">{{ t.next }}</p>

      <div class="mt-6 flex flex-col gap-2">
        <NuxtLink to="/app/purchase-requests" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl">
          {{ t.viewMine }}
        </NuxtLink>
