    ...
    <div class="bg-white/90 backdrop-blur-sm shadow-sm border-b border-gray-100">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div class="flex items-center gap-4">
          <NuxtLink :to="`/app/shopping/campaigns/${route.params.id}`"
...
const fetchCampaign = async () => {
  ...
  try {
    ...
    if (data.status !== 'draft') {
      await navigateTo(`/app/shopping/campaigns/${route.params.id}`)
      return
    }
...
const handleSubmit = async () => {
  ...
  try {
    await $customFetch(`/shopping/campaigns/${route.params.id}`, {
      method: 'PUT',
      body: {
