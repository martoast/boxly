...
const onSubmit = async ({ fields, images, variants }) => {
  ...
  try {
    // 1. Create product
    const createRes = await $customFetch('/shopping/products', {
      method: 'POST',
      body: fields,
    })
    ...
    if (variants && variants.length > 0) {
      await $customFetch(`/shopping/products/${product.id}/variants/sync`, {
        method: 'POST',
        body: { variants },
      })
    }
    ...
    if (images && images.length > 0) {
      ...
      await $customFetch(`/shopping/products/${product.id}/images`, {
        method: 'POST',
        body: fd,
      })
    }
    ...
    router.push(`/app/shopping/products/${product.id}/edit`)
  } catch (err) {
    console.error(err)
