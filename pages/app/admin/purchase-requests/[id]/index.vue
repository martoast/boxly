                ...
                <div class="flex gap-4 items-start">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <!-- Cost breakdown form (store PRs in pending_review only) —
                         admin enters source-store tax/shipping + commission %
                         after running the actual checkout. Submitting flips
...
// breakdown isn't saved yet). This is the number admin sees as the
...
// Tracks per-item update spinners so we can disable buttons mid-request
const stockUpdating = ref({});

// Default Boxly commission % — pre-fills the per-item input. Mirrors
// services.commission.default_percent on the API.
...
const saveCostBreakdown = async (item) => {
  ...
  try {
    ...
    const resp = await $customFetch(
      `/admin/purchase-requests/${request.value.id}/items/${item.id}/cost-breakdown`,
      {
        method: 'PUT',
        body: {
...
const markStockStatus = async (item, stockStatus) => {
  ...
  try {
    const resp = await $customFetch(
      `/admin/purchase-requests/${request.value.id}/items/${item.id}/stock-status`,
      { method: 'PUT', body: { stock_status: stockStatus } },
    );
    // Patch in place so the UI updates without a full re-fetch
    const next = resp?.data || resp;
    const idx = request.value.items.findIndex(x => x.id === item.id);
    if (idx >= 0 && next) {
      request.value.items[idx] = { ...request.value.items[idx], ...next };
...
const onCreateQuoteClick = async () => {
  ...
  // Confirmation prompt for store PRs
