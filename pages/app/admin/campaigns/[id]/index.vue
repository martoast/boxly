      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div class="flex items-center gap-3">
            <NuxtLink to="/app/admin/campaigns"
          ...
          <div v-if="campaign" class="flex gap-2">
            ...
            <template v-if="campaign.status === 'draft'">
              ...
              <NuxtLink :to="`/app/admin/campaigns/${campaign.id}/edit`"
                class="px-4 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-xl border border-gray-300 hover:bg-gray-50 shadow-sm transition-all duration-300">
                {{ t.edit }}
              </NuxtLink>
...
const fetchCampaign = async () => {
  loading.value = true
  try {
    const response = await $customFetch(`/admin/campaigns/${route.params.id}`)
...
const fetchRecipients = async (page = 1) => {
  ...
  try {
    const response = await $customFetch(`/admin/campaigns/${route.params.id}/recipients`, {
      params: { page }
    })
...
const fetchAudiencePreview = async (page = 1) => {
  ...
  try {
    const response = await $customFetch(`/admin/campaigns/${route.params.id}/preview-recipients`, {
      params: { page }
    })
...
const startCampaign = async () => {
  ...
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}/start`, { method: 'POST' })
...
const pauseCampaign = async () => {
  actionLoading.value = true
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}/pause`, { method: 'POST' })
    await fetchCampaign()
  } catch (error) {
...
const resumeCampaign = async () => {
  actionLoading.value = true
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}/resume`, { method: 'POST' })
    await fetchCampaign()
  } catch (error) {
...
const cancelCampaign = async () => {
  actionLoading.value = true
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}/cancel`, { method: 'POST' })
...
const deleteCampaign = async () => {
  actionLoading.value = true
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}`, { method: 'DELETE' })
    await navigateTo('/app/admin/campaigns')
  } catch (error) {
