    ...
    <div class="bg-white/90 backdrop-blur-sm shadow-sm border-b border-gray-100">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div class="flex items-center gap-4">
          <NuxtLink :to="`/app/admin/campaigns/${route.params.id}`"
          ...
          <h1 class="text-xl sm:text-2xl font-extrabold text-gray-900">{{ t.editCampaign }}</h1>
        </div>
      </div>
    </div>
...
const translations = {
  editCampaign: { es: 'Editar Campaña', en: 'Edit Campaign' },
...
const fetchCampaign = async () => {
  ...
  try {
    ...
    if (data.status !== 'draft') {
      await navigateTo(`/app/admin/campaigns/${route.params.id}`)
      return
    }
    ...
    form.value = {
      ...
      batch_delay_seconds: data.batch_delay_seconds || 60,
    }
...
const fetchAudiencePreview = async () => {
  ...
  try {
    const response = await $customFetch('/admin/campaigns/preview-audience', {
...
const handleSubmit = async () => {
  ...
  try {
    await $customFetch(`/admin/campaigns/${route.params.id}`, {
      method: 'PUT',
      body: {
