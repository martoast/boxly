  ...
  <section class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="flex items-center justify-between gap-4 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900">Tiendas</h1>
          <p class="text-sm text-gray-500 mt-1">Marcas / fuentes de las que curamos productos</p>
        </div>
      ...
      <div v-else-if="stores.length === 0" class="bg-white rounded-2xl border border-gray-100 p-12 text-center">
        <p class="text-gray-700 font-semibold">Sin tiendas</p>
        <p class="text-gray-400 text-sm mt-1">Crea tu primera tienda (marca) para empezar.</p>
      </div>
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <th class="hidden sm:table-cell px-4 py-3">Slug</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="s in stores" :key="s.id" class="hover:bg-gray-50">
              <td class="px-3 sm:px-4 py-3">
                <div class="flex items-center gap-3 min-w-0">
                  <div class="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center text-xs text-gray-400">
                    <img v-if="s.logo_url" :src="s.logo_url" :alt="s.name" class="w-full h-full object-cover" />
              ...
              <td class="hidden sm:table-cell px-4 py-3 text-gray-500 font-mono text-xs">{{ s.slug }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-right">
                <NuxtLink :to="`${editBasePath}/${s.id}/edit`" class="inline-flex items-center justify-center -my-1 px-3 py-2 rounded-lg text-primary-600 font-medium hover:bg-primary-50 text-sm">Editar</NuxtLink>
              </td>
            </tr>
          </tbody>
        </table>
...
const editBasePath = computed(() => route.path.includes('/shopping/') ? '/app/shopping/stores' : '/app/admin/stores')
...
const fetchStores = async () => {
  loading.value = true
  try {
    const res = await $customFetch(`${apiNs.value}/stores`, {
...
watch(search, () => {
  ...
  searchTimer = setTimeout(fetchStores, 300)
})
