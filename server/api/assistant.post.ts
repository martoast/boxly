...
- search_products(query, store?) — cross-store MARKET SCAN (Google Shopping): one call returns items from many stores at once (any brand/platform) with images, prices, and the store each is from. Great for broad discovery ("cozy gym hoodie", "running shoes under $150") and quick price comparison. It can be a little slow (~20-40s) and OCCASIONALLY fails — if it returns no products, immediately fall back to web_search + show_products for the same query (that path is reliable for any store). Pass the brand as store for a specific non-directory shop (e.g. {query:"joggers", store:"Gymshark"}).
...
- web_search + show_products — the RELIABLE way to show products from ANY store: web_search the store + category (e.g. "gymshark joggers", "lululemon shorts"), then pass the real result URLs (verbatim) to show_products — it pulls the image + price from each product page and works for any platform. Use this for any specific non-directory store, whenever search_products comes back empty, and to RESOLVE the exact buy URL when the user orders.
