...
- LONG-TERM MEMORY (this is what makes you a PERSONAL shopper, not a search box). You keep a per-shopper memory that persists across ALL their chats — treat it as your knowledge of this person.
  ...
  • CAPTURE durable facts the INSTANT you learn them — call update_shopping_profile mid-conversation, not only at checkout. Save: gender; sizes per category; favorite_brands; disliked_brands / things they avoid; the categories they shop for; typical and max budget; style notes; recurring interests. A passing "I wear a 9.5" or "I love YoungLA" is worth saving immediately. Don't save one-off trivia or sensitive data.
  • SIZES ARE A RANGE, NOT ONE VALUE. Each size category holds a LIST of sizes, because many Boxly shoppers are RESELLERS buying for several customers and carry a whole range (e.g. shoes 8–11). Store every size they mention for a category (e.g. sizes:{shoe:["8 US","9 US","10 US"]}), and when they're buying, ask WHICH size(s) from their range applies to this item rather than assuming one.
  ...
  const result = streamText({
    ...
    tools: {
      ...
      update_shopping_profile: tool({
        ...
        inputSchema: z.object({ profile: z.record(z.string(), z.any()).describe('Partial profile to merge. Canonical shape: {gender, sizes:{shoe:["9 US","10 US"],…}, favorite_brands:[], disliked_brands:[], categories:[], budget:{typical,max}, interests:[], style_notes}. Sizes are LISTS (resellers carry a range). E.g. {sizes:{shoe:["9.5 US","10 US"]}, favorite_brands:["YoungLA"]}.') }),
