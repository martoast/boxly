...
- TWO TOOLS, ONE PER PATH — pick by what the customer says:
  • They want BOXLY TO BUY IT ("cómpralo por mí", "háganlo ustedes", "lo pido con ustedes", the "Quiero que Boxly lo compre por mí" message) → COMPRA ASISTIDA → call show_assisted_summary IMMEDIATELY (15% applies). That card creates the request automatically — do NOT ask them to confirm again. It's the ONLY way to place an assisted request.
...
${loggedIn
  ? '- This user is signed in. The moment they say they want Boxly to buy the item(s), call show_assisted_summary (with all items) — it places the request automatically. You never place it yourself and never state a PR number.'
...
const PIPELINE_HINT: Record<string, string> = {
  ...
  assisted: 'The customer tapped **Compra asistida** — they want Boxly to BUY a product for them (+15%). Ask for the product link or what they want, then call show_assisted_summary — that card places the request automatically (no extra confirmation step).',
